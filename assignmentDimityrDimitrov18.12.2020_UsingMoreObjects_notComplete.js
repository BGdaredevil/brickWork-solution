function brickWork(input) {
  ...
  class Brick {
    ...
    getVertical(list) {
      let returnBrick = list.find(
        (brick) => brick.direction === "vertical" && brick.isUsed === undefined
      );
      Object.defineProperty(brick, "isUsed", { value: true });
      return returnBrick;
    }
    getHorizontal(list) {
      return list.find((brick) => brick.direction === "horizontal");
    }
  }

  for (let i = 0; i < botRow.length; i++) {
    for (let j = 0; j < botRow[i].length; j++) {
      if (
        bricks.every((el) => {
          el.number === undefined;
        })
      ) {
        if (botRow[i][j] === botRow[i][j + 1]) {
          bricks.push(new Brick(botRow[i][j], "horizontal"));
        } else {
          bricks.push(new Brick(botRow[i][j], "vertical"));
        }
      } else if (bricks.every((el) => el.number !== botRow[i][j])) {
        if (botRow[i][j] === botRow[i][j + 1]) {
          bricks.push(new Brick(botRow[i][j], "horizontal"));
        } else {
          bricks.push(new Brick(botRow[i][j], "vertical"));
  ...
  for (let i = 0; i < botRow.length; i++) {
    for (let j = 0; j < botRow[i].length; j++) {
      for (let brick of bricks) {
        let brNumber = brick.number;
        let brDirect = brick.direction;
        if (topRow[i][j] === 0) {
          switch (brDirect) {
            case "horizontal":
              let item = brick.getVertical(bricks);
              if (item !== undefined) {
                topRow[i][j] = brick.number;
                topRow[i + 1][j] = brick.number;
              } else {
                item = brick.getHorizontal(bricks);
                topRow[i][j] = brick.number;
                topRow[i + 1][j] = brick.number;
              }
              break;
